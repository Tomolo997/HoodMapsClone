...
var circleZoomMax = 18;
var circleZoomMin = 10;
var map;
  ...
  return _regeneratorRuntime.default.wrap(function _callee2$(_context2) {
    while (1) {
      switch (_context2.prev = _context2.next) {
        case 0:
          _context2.next = 2;
          return getData(inputSearch.value);
...
var undrawButton = document.querySelector('.undraw');
undrawButton.addEventListener('click', function () {
  canvas.style.zIndex = 1;
  mapP.style.zIndex = 1;
  addTextBoolean = false;
});
var addTextBoolean = false;
var lat, lng;
var position;
var positionOnMap;
var isDrawing = false;
...
drawButton.addEventListener('click', function () {
  ...
  mapP.style.zIndex = -1; //getMouse position

  position = {
    x: 0,
    y: 0
  };
  var yea = map.getPixelBounds();
  console.log(yea);
  positionOnMap = {
    position: [{
      positionX: 0,
  ...
  var changeColor = document.querySelectorAll('.changeColor');
  var color = 'red';
  ctx.strokeStyle = 'red';
  changeColor.forEach(function (el) {
  ...
  var mouseDraw = function mouseDraw(e) {
    if (!isDrawing) return;
    ctx.lineCap = 'round';
    ...
    */
    ...
    getMousePosition(e); //ctx.lineTo(position.x, position.y);
    ...
    var _map$containerPointTo = map.containerPointToLatLng([position.x, position.y]),
    ...
    L.circle([positionOnMap.position.positionX, positionOnMap.position.positionY], {
      color: color,
      radius: desiredRadius,
      opacity: 0.5,
      fillOpacity: 1
    }).addTo(map); //console.log(position);
    //console.log(positionOnMap);

    var yea = map.latLngToLayerPoint([lat, lng]);
    var xCoords = yea.x;
    var yCoords = yea.y;
